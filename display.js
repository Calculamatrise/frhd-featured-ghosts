        ...
        for (let player in players) {
            for (const ghost in players[player]) {
                ...
                for (const element of document.getElementsByClassName("track-leaderboard-race")) {
                    ...
                    let name;
                    if (element.innerText.toLowerCase() == ((name = alias[player] && Object.entries(alias[player]).filter(([,b]) => b == ghost.slice(-b.length))?.[0]?.[1] || player) && name.length > 15 && name.slice(0, 12) + "..." || name)) {
